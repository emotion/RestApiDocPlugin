...
public class ShowRestServicesAction extends AnAction {
    ...
    private void registerToolWindow(Project project, RestServicesUI ui) {
        ...
        boolean isToolWindowRegistered = Arrays.stream(toolWindowIds).anyMatch(s -> s.equalsIgnoreCase(TOOL_WINDOW_ID));

        if (!isToolWindowRegistered) {
            ToolWindow toolWindow = ToolWindowManager.getInstance(project).registerToolWindow(TOOL_WINDOW_ID, false, ToolWindowAnchor.RIGHT);
